...
1. **首页与价值介绍**：展示从小说到结构化剧本的一站式改编流程。
2. **导入小说**：载入示例或上传文件，展示自动章节识别和章节调整。
3. **故事分析**：展示人物、关系、故事概要、章节关键事件与改编设置。
4. **逐章生成**：展示真实生成进度、单章重新生成和对白驱动结果。
5. **剧本工作台**：编辑场景、动作与对白，使用 AI 润色并对照原文。
6. **结构化导出**：展示实时 YAML 预览并下载剧本文件。
