...
- 初稿默认减少旁白、优先通过人物对白推进剧情；对白密度和目标场景数可继续调整
- 后端逐章生成并立即保存结构化剧本，支持失败重试和单章重新生成
- 在三栏工作台中按演出顺序混排动作与对白，每条对白都带对应动作
...
| 剧本生成 | 已完成 | 逐章生成对白驱动的剧本初稿，减少旁白，目标场景数按章节分配，支持失败重试和单章重新生成 |
| 剧本编辑工作台 | 已完成 | 支持人物及引用同步、关系与章节分析编辑、章节划分、场景归属，以及动作与对白统一排序 |
| YAML 导出 | 已完成 | 使用 `storyforge-script/v2` 正式 Schema，按演出顺序输出剧本内容并校验每条对白的对应动作 |
