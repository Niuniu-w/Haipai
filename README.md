...
| 剧本生成 | 已完成 | 后端逐章调用模型并立即保存，展示真实章节进度，支持失败重试和单章重新生成 |
| 剧本编辑工作台 | 已完成 | 支持添加人物与出场人物、调整章节划分和对白顺序，并在删除场景前二次确认 |
| YAML 导出 | 已完成 | 支持实时预览、完整结构校验、服务端导出、复制和下载 |
