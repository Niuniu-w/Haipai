...
项目中的 `dialogueDensity` 和 `targetSceneCount` 会参与生成：新项目默认使用密集对白，提示词要求减少旁白并优先用对白推进剧情；目标总场景数按章节分配且每章至少一场。

通用项目更新接口使用 `revision` 乐观锁。客户端从项目响应读取 `data.revision`，更新时通过 `If-Match` 请求头传回；版本过期返回 `409`，缺少版本号返回 `428`。

YAML 导出使用 `storyforge-script/v2` 正式 Pydantic/JSON Schema。场景使用按演出顺序排列的 `content` 列表混排动作与对白，每条对白必须带对应动作、人物、情绪和台词；同时校验作品标题、唯一章节与场景编号、每章至少一个场景、来源章节、人物关系和出场人物引用。结构无效时返回 `422` 和具体错误列表，校验通过后才返回 UTF-8 YAML 文件。
